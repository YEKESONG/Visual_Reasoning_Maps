...
本机运行的学术阅读原型：从 PDF 或 arXiv HTML 重建分层推理图，点击步骤或连线查看原文、解释和修改建议。中、英、法三语界面；附法语综述初稿与中文过程文档。

![流程图与原文](docs/screenshots/map-pdf.png)

## 语言切换

右上角选择 **中文 / English / Français**，立即切换整个界面，并在刷新后记住偏好。首次进入跟随浏览器语言（其他语言回退法语）。图谱、详情、术语卡、原文导航、错误与进度提示、键盘辅助标签均支持三语；人工演示的论点与说明同步切换，引文与原文保留原语言。

上传区的“分析语言”可选跟随界面、跟随原文或指定中/英/法语。该选项用于新分析；已有模型生成图谱保留生成语言，按需解释使用当前界面语言。不同语言的图谱和解释独立缓存，不会覆盖或误用旧结果。生成内容仍需 API key；切换界面和演示无需联网或调用模型。
