...
## S21d 文档与网页端完整验收
- 网页端验收（8010 端口测试服务，最新代码）：
  - 中文：把此前在临时目录跑出的模型缓存合并到 data/ 中对应文档，再在网页上传同一 PDF。所有阶段命中缓存，0.5 秒完成并跳转到图谱：8 个主步骤、46 条关系，主旨、节点、原版片段和高亮位置均正常。
  - 英文：在网页选择图谱语言 English 后上传，全新调用模型。各阶段耗时约为：骨架 72 秒、分节 17 秒、跨节 86 秒、核对 22 秒、两轮修复 38 秒、建议 9 秒，共约 4 分钟；26 次调用、30.4 万 token、0.066 美元。结果 9 个主步骤、32 个子步骤、48 条关系、6 张术语卡；41 个步骤中 39 个已核对，48 条关系中 30 条已核对、15 条部分支持、3 条待核实；最终只剩 1 个结构问题（某步骤子步骤过多）。页面无脚本错误。
  - 两次运行的模型意见都指出：摘要写"采石成功率最多提升 45%、建柱 30%"，正文写"90.0%、54.5%"，口径需要统一或说明。
  - 这次运行暴露的格式问题在 S21a–S21c 修复。
- 文档：
  - README（法文）和 README.zh-CN 重写：说明一次分析的四个阶段、真实耗时和费用、怎么看图、语言、全部配置项（新增 LLM_THINKING_MAX_TOKENS、SECTION_CHUNK_CHARS）、数据去向、与 CI 相同的检查命令、端到端测试的 BASE_URL、已知限制。删去"atelier de lecture""阅读工作台"之类的说法，改为直接描述功能。
  - .env.example 增加两个新配置项。
  - ARCHITECTURE：B1/B3/B4 的新职责、GET /api/tasks/{id}、health 返回模型名、模型输入格式、修复流程和状态含义。
  - DECISIONS：新增 ADR-010（PDF 版面规则）至 ADR-014（示例副本同步）。
...
## S21f 阅读卡片去模板化
- 问题：24 张阅读卡片的"Point mobilisable"一行与"Méthode"逐字相同，"待核实"一行也全部是同一句"核对全文的具体页码、评价协议与适用边界"，是批量生成时留下的模板痕迹，看不出每篇文献在综述里用来支持什么。
- 完成内容：字段改名为"À citer pour"，逐篇写明在综述中引用它来支持哪一点，内容只取自卡片已有字段和综述正文中已核实的表述，没有新增事实；"待核实"按文献类型写成具体事项（评测类：实验协议与页码；综述类：分类维度的原文表述；方法类：方法细节；工具类：版本号与许可证页面），保留原有的专项待办（文献 4、10、11b、16）。
- 验证：脚本确认每张卡片恰好一行"À citer pour"，不再有与其他字段重复的行；逐张通读。

## S22a 服务器内部错误的提示
- 问题：更新代码后，用户在 8000 端口运行的仍是更新前启动的服务进程，它用旧的数据模型读取新生成的文件：parsed.json 里词的 `page` 字段（S19a 新增）和 metadata 的 `language` 字段（S19e 新增）被旧模型当作不允许的多余字段，`/api/docs/{id}` 和 `/api/docs/{id}/flow` 返回 500（响应校验失败，正文是纯文本 "Internal Server Error"）。前端只在返回体带 `detail` 时显示服务器的说明，否则一律显示"请求失败，请重试"，重试不会有任何变化，也看不出该去哪里找原因。
- 复现：在临时 worktree 中检出 S18 的代码，指向复制出的新数据运行，上面两个接口返回 500，日志为 5742 个 `extra_forbidden`（paragraphs.words.page）和 1 个（metadata.language）；当前代码对同一数据全部返回 200。
...
## S22d README 流程图适配 GitHub 正文宽度
- 问题：S22c 的流程图是 8 个节点横排一行，渲染后原始宽度 2043px。GitHub 会把 Mermaid 图缩放到正文宽度（桌面端约 850px），文字缩到原来的四成左右，基本看不清。
- 排查：用 mermaid 11 渲染几种布局并读取 SVG 的 viewBox：竖排一列 276×966（太长）；两行但用节点直接相连时，子图的 `direction LR` 会被忽略，仍排成一行（2143px）；三组子图上下排列、组内横排、组与组之间用子图相连，宽 770–862px、高 536px。
- 完成内容：三份 README 改用三组布局：1 读取（PDF 或 arXiv 链接 → 文字与版面 → 句子编号）、2–4 模型与核对（重建推理 → 核对与定向修正 → 修改建议）、结果（flow.json → 浏览器中的图谱）。分组编号对应图下方的四个阶段说明。
- 验证：三张图都能解析并渲染出 8 个节点，宽度 770–862px；推送后在 GitHub 主页查看渲染效果。
- 主要文件：README.md、README.fr.md、README.zh-CN.md。

## S23a 综述文献逐篇对照原文核实
- 问题：综述初稿只读过摘要和出版信息（初稿摘要里也这样写），阅读卡片每项只有一句话，例如研究问题写成"怎样跟踪人物和主题？"；参考文献全部是 `@misc`，多数引用预印本，缺卷期页码；文献 4 的发表状态、文献 11b 的代码仓库等待核实项一直没有结论。
- 做法：
  - 取得原文：arXiv、ACL Anthology 和作者或机构仓库的开放 PDF 由网页读取工具保存到本地，再用 PyMuPDF 按页转成文本，用关键词定位段落，逐条核对要引用的事实和数字，页码换算成期刊或会议论文集的正式页码（预印本用所读版本的页码）。
  - 出版信息逐条对照 Crossref，参考文献改为正式发表版本（例如 Story Ribbons 改为 TVCG 32(1)，Semantic Reader 改为 CACM 67(10)，SciDaSynth 改为 Campbell Systematic Reviews 21(4)），`note` 字段注明实际读的版本。
  - 25 张阅读卡片重写：阅读范围、研究问题、方法、带页码的结果、作者自述的局限、与本项目的关系、可引用的要点、待核实事项。原清单第 12 项附带的边绑定综述作为 12b 纳入。
  - `reference_index.json` 由卡片重新生成，增加"阅读范围"字段。
- 结果：21 篇读了全文或针对性阅读；4 篇拿不到开放全文（Khartabil、Sugiyama、KDEEB、边绑定综述），只按摘要核实并在卡片中注明。已结案的待核实项：文献 4 的 arXiv v2 已是 CGF 45(3) STAR 排版；文献 11b 的代码链接原来指向 fork 的 fork，改为原仓库；文献 16 的题名按期刊版更正。
