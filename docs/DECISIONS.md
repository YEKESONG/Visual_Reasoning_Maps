...
## ADR-015 README 以英文为默认（S22c）

- 背景：原始任务说明（AGENT_PROMPT 中的 `readme_languages: [fr, zh-CN]`）要求 `README.md` 用法语、另附中文版。GitHub 仓库主页只显示 `README.md`，不读法语的访问者无从了解项目。
- 决定：应用户要求，`README.md` 改为英文，法文版改名为 `README.fr.md`，中文版仍为 `README.zh-CN.md`；三份结构相同，顶部互相链接。界面的源语言仍是法语，其余项目文档仍用中文。
- 影响：修改 README 时三份要同步更新。AGENT_PROMPT 作为原始任务记录保持原样，这里记录与它的差异。
