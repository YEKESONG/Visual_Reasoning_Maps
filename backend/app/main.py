...
def sync_examples(examples: Path, target: Store) -> None:
    """Copy the bundled examples into the data folder, replacing outdated copies.

    Example folders are generated by scripts/build_examples.py and never edited by users.
    """
    for example in examples.glob("demo-*"):
        destination = target.directory(example.name)
        if not example.is_dir():
            continue
        current = destination / "flow.json"
        if current.exists() and current.read_bytes() == (example / "flow.json").read_bytes():
            continue
...
async def lifespan(app: FastAPI):
    sync_examples(ROOT / "examples", store)
    yield
